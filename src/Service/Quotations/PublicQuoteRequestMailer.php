...
final class PublicQuoteRequestMailer
{
 ...
 public function send(QuoteRequest $request):void{$email=(new TemplatedEmail())->from(new Address($this->fromAddress,$this->fromName))->to(new Address((string)$request->getEmail(),(string)$request->getFullName()))->subject(sprintf('Cotización demostrativa %s | PrintFlow',$request->getFolio()))->htmlTemplate('emails/quotations/public_quote_request.html.twig')->context(['request'=>$request,'privacyResponsible'=>$this->privacyResponsible,'privacyAddress'=>$this->privacyAddress,'privacyEmail'=>$this->privacyEmail,'privacyUrl'=>$this->urlGenerator->generate('app_privacy_notice',[],UrlGeneratorInterface::ABSOLUTE_URL),'acceptanceUrl'=>$this->urlGenerator->generate('quotation_public_accept',['token'=>$request->getPublicToken()],UrlGeneratorInterface::ABSOLUTE_URL)])->attach($this->pdf->render($request),$this->pdf->filename($request),'application/pdf');$this->mailer->send($email);}
}
