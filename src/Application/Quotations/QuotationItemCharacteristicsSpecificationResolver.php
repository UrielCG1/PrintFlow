...
final class QuotationItemCharacteristicsSpecificationResolver
{
    ...
    public function resolve(
        ...
        foreach ($configurations as $configuration) {
            ...
            $submittedValue = $this->submittedCharacteristicValue(
                $characteristic,
    ...
    /** @param array<string, mixed> $submittedSpecifications */
    private function submittedCharacteristicValue(
        ...
        $value = match ($characteristic->getCode()) {
             ...
             * característica configurada; así una clave genérica vacía no puede
             * ocultar el ancho/alto realmente capturado.
             *
             * El fallback a characteristic_* conserva compatibilidad con
             * formularios/snapshots previos donde no exista la clave dedicada.
             */
            'FINISHED_WIDTH_CM' => array_key_exists('finished_width_cm', $submittedSpecifications)
                ? $submittedSpecifications['finished_width_cm']
                : ($submittedSpecifications[$fieldKey] ?? ''),
            'FINISHED_HEIGHT_CM' => array_key_exists('finished_height_cm', $submittedSpecifications)
